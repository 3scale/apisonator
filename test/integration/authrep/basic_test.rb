...
class AuthrepBasicTest < Test::Unit::TestCase
  ...
  test_authrep 'authorizes if there are app and user limits and none are exceeded' do |e|
    app_daily_limit = 10
    user_daily_limit = 2
  ...
  test_authrep 'denies auth if there are app and user limits and only app limits are exceeded' do |e|
    app_daily_limit = 10
  ...
  test_authrep 'denies auth if there are app and user limits and only user limits are exceeded' do |e|
    user_daily_limit = 10
    hits_to_report = user_daily_limit + 1
    app_daily_limit = hits_to_report + 1
    fixtures = limited_app_and_user!(
        @service, @metric_id, app_daily_limit, user_daily_limit)
    ...
    get e, :provider_key => @provider_key,
           ...
           :app_id => fixtures[:app].id,
           :user_id => fixtures[:user].username,
           :usage => { 'hits' => app_daily_limit + 1 }

    assert_not_authorized('usage limits are exceeded')
  end

  test_authrep 'denies auth if there are app and user limits and both are exceeded' do |e|
    app_daily_limit = user_daily_limit = 1
    fixtures = limited_app_and_user!(
        @service, @metric_id, app_daily_limit, user_daily_limit)
