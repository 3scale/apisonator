...
class OauthBasicTest < Test::Unit::TestCase
  ...
  test_nobody :get, '/transactions/oauth_authorize.xml' do
    { provider_key: @provider_key, app_id: @application.id, log: @apilog }
  end
  ...
  test 'authorizes if there are app and user limits and none are exceeded' do
    app_daily_limit = 10
    user_daily_limit = 2
  ...
  test 'denies auth if there are app and user limits and only app limits are exceeded' do
    app_daily_limit = 10
  ...
  test 'denies auth if there are app and user limits and both are exceeded' do
