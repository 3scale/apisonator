...
class StorageFailoverTest < Test::Unit::TestCase
  include TestHelpers::EventMachine::Methods
  ...
  def test_failover_under_parallel_access
    start_redis_server(6400)
    start_redis_server(6401)

    redis_send(6400, 'set one one_6400')
    redis_send(6401, 'set one one_6401')
    
    redis_send(6400, 'set two two_6400')
    redis_send(6401, 'set two two_6401')

    storage      = nil
    response_one = nil
    response_two = nil

    # This will tell me if both requests are done.
    done = lambda { !response_one.nil? && !response_two.nil? }

    EM.run do
      Fiber.new do
        storage = Storage.connect(:servers => ['127.0.0.1:6400', '127.0.0.1:6401'])

        # Run the requests in two "parallel" fibers.

        Fiber.new do
          response_one = storage.get('one')
          EM.stop if done.call
        end.resume

        Fiber.new do
          response_two = storage.get('two')
          EM.stop if done.call
        end.resume
    ...
    assert_equal 'one_6401', response_one
    assert_equal 'two_6401', response_two
  end
